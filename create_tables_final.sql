...
LEFT JOIN annp_final.enrolls e ON e.athleteid = a.athleteid
LEFT JOIN annp_final.result r ON r.license = e.license
LEFT JOIN annp_final.split ss ON ss.resultid = r.resultid
LEFT JOIN annp_final.session s ON s.meetid = e.meetid
LEFT JOIN annp_final.event ev ON ev.sessionid = s.sessionid
GROUP BY a.athleteid, e.meetid, r.resultid, from_club
...
ALTER TABLE annp_final.athlete_defacto ADD CONSTRAINT fk_result FOREIGN KEY (resultid) REFERENCES annp_final.result (resultid) ON DELETE NO ACTION ON UPDATE NO ACTION;
