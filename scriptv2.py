...
def create_annp_final_inputs(data, id_key):
    ...
    for club in data['CLUBS']['CLUB']:
        clubid = club['@code'] + "-" + club['@nation'] + "-" + str(club['@region'])

        clubs.append({'clubid': clubid, 
                      'code': club['@code'], 
                      'name': club['@name'], 
                      'nation': club['@nation'], 
                      'region': club['@region']}) 
        ...
        for idx, athlete in enumerate(club['ATHLETES']['ATHLETE']):
            ...
            #license = str(athlete['@firstname']) + "-" + str(athlete['@lastname']) + "-" + clubid + "-" + str(id_key)
            athletes.append({
                'athleteid': athleteid,
        ...
        for session in data['SESSIONS']['SESSION']:
            ...
            sessionid = str(id_key) + "-" + str(k)
            if sessionid not in sessions_prov:
                ...
                sessions_prov.append(sessionid)
                k += 1
            else:
                pass
            ...
            for event in session['EVENTS']['EVENT']:
                ...
                eventid = str(sessionid) + "-" + str(swimstyleKey) + "-" + event['@gender']

                if eventid not in event_prov:
                    ...
                    event_prov.append(eventid)
                    event_dict[event['@eventid']] = eventid
    ...
    for club in data['CLUBS']['CLUB']:
        ...
        for idx, athlete in enumerate(club['ATHLETES']['ATHLETE']):
            ...
            for result in athlete['RESULTS']['RESULT']:
                ...
                for event_orig, my_event in event_dict.items():

                    resultid = my_event + "-" + license

                    if str(result['@eventid']) == str(event_orig):
...
def generate_sql_annp_final_enrolls(enrolls):
    sql = ''
    for e in enrolls:
        sql += "INSERT INTO annp_final.enrolls (meetid, license, athleteid, clubid) VALUES ('"
        sql += str(e['meetid'])
        sql += "', '"
        sql += e['license']
        sql += "', '"
...
class Table:
    ...
    def create_insert_data(self, primary_key, parent_table_name, parent_table_val, data, db_name):        
        ...
        s += 'INSERT INTO OR IGNORE {db_name}.{name} ('.format(db_name=db_name, name=self.name)
