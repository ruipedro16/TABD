...
def xml_to_sql(folders_path):
    ...
    i = 0
    for json_file, file_path in zip(json_files, json_file_path):
        sql_annp_final_inputs = create_annp_final_inputs(json_file['LENEX']["MEETS"]['MEET'], id_key + i)
...
def create_annp_final_inputs(data, id_key):
    ...
    # Initially get sessionid to populate eventId from results in athletes

    athletes_prov = []
    for club in data['CLUBS']['CLUB']:

        clubid = club['@code'] + "-" + club['@nation'] + "-" + club['@region']
        ...
        # ATHLETES
        id_ath = 0
        for athlete in club['ATHLETES']['ATHLETE']:

            athleteid = athlete['@firstname'] + "-" + athlete['@lastname'] + "-" + str(athlete['@birthdate']) + "-" + str(id_ath)

            #license = athlete['@license'] if '@license' in athlete else str(int(id_key) + int(club['@clubid']))

            if athleteid not in athletes_prov:
                ...
                athletes_prov.append(athleteid)
            ...
            for result in athlete['RESULTS']['RESULT']:
                ...
                eventid = str(id_key) + "-" + result['@eventid']
                
                # Each result needs a unique id which should be unique over a meet.
                resultid = str(id_key) + "-" + result['@resultid']
                ...
                results.append({
                    ...
                    'eventid': eventid,
                    'license': license,
                    'points': result['@points'] if '@points' in result else '0',
            ...
            for split in result['SPLITS']['SPLIT']:
                ...
                splitid = str(id_key) + "-" + str(len(splits))
                splits.append({
                    'splitid': splitid,
                    'resultid': resultid,
    ...
    for session in data['SESSIONS']['SESSION']:
        ...
        sessionid = str(id_key) + "-" + str(len(sessions))
        ...
        for event in session['EVENTS']['EVENT']:
            ...
            eventid = str(id_key) + "-" + str(event['@eventid'])

            events.append({
                'eventid': eventid,
                'gender': event['@gender'],
                'time': event['@daytime'] if '@daytime' in event else '00:00' ,
                'sessionid': sessionid,
            ...
            if 'AGEGROUPS' not in event:
                continue
            if 'AGEGROUP' not in event['AGEGROUPS']:
                continue
            if '@agegroupid' in event['AGEGROUPS']['AGEGROUP']:
                event['AGEGROUPS']['AGEGROUP'] = [event['AGEGROUPS']['AGEGROUP']]

            for agegroup in event['AGEGROUPS']['AGEGROUP']:
    ...
    sql += generate_sql_annp_final_agegroups(agegroups)
    sql += generate_sql_annp_final_rankings(rankings)
    return sql
...
def generate_sql_annp_final_athletes(athletes):
    sql = ''
    for a in athletes:
        sql += "INSERT INTO annp_final.athlete (athleteid, firstname, lastname, gender, birthdate, nation) VALUES ('"
        sql += a['athleteid']
        sql += "', '"
        sql += a['firstname']
        sql += "', '"
...
def generate_sql_annp_final_enrolls(enrolls):
    sql = ''
    for e in enrolls:
        sql += "INSERT INTO annp_final.enrolls (license, meetid, athleteid, clubid) VALUES ('"
        sql += e['license']
        sql += "', '"
        sql += str(e['meetid'])
        sql += "', '"
...
def generate_sql_annp_final_agegroups(agegroups):
    sql = ''
    for s in agegroups:
        sql += "INSERT INTO annp_final.agegroup (agegroupid, agemax, agemin, eventid) VALUES ('"
        sql += s['agegroupid']
        sql += "', '"
        sql += s['agemax']
        sql += "', '"
...
def generate_sql_annp_final_rankings(rankings):
    sql = ''
    for s in rankings:
        sql += "INSERT INTO annp_final.rankings (rankingid, order, place, resultid, agegroupid) VALUES ('"
        sql += s['rankingid']
        sql += "', '"
        sql += s['order']
        sql += "', '"
...
class Table:
    ...
    def __str__(self):
        ...
        if self.name in ['athlete', 'club', 'pool', 'meet', 'session', 'event', 'result', 'split', 'swimstyle', 'agegroups', 'rankings']:
